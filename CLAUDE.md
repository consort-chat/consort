...
`matrix.to` links that go where they point, editing a message, both sent and
drawn, and an icon in the system tray that brings the window back. Deleting your
own message is not built, and neither is upload progress, a thumbnail for a clip
somebody sends, or a count on the icon when something is unread.
